...
No watch, erro esperado de leitura, configuração inválida ou lock ocupado produz quadro `estado indisponível`, sem reutilizar linhas antigas como atuais nem revelar erro bruto. A próxima atualização tenta novamente; falha persistente continua explícita e requer diagnóstico, não reinício automático de jobs. Erros de argumentos continuam recusados. O painel lê metadados sem recalcular hashes das fontes a cada refresh e preserva recuperação de supervisor órfão. Ainda percorre metadados históricos; não é índice ou daemon novo. `result`/`accept` mantêm validações completas das entradas, e acompanhar o painel não as substitui.
